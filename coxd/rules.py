"""Compounding repo-rules (ported from cox/rules.py, DESIGN-V35).

Cherny's "write the rule, not the correction": every review finding or captain
correction can become a durable one-line rule so the implementer stops
repeating it. Rules live in coxd's registry home (never the per-task worktree,
which is thrown away), keyed by repo, and are injected into every future
implementer brief for that repo. Append-only and captain-curated — mirrors the
"typed needs-human, no dumping ground" discipline, so this file stays a short
list of real lessons.
"""
...
def rules_path(repo: str) -> Path:
    ...
    return registry.home() / "rules" / f"{safe}.md"
